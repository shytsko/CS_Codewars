...
// Going to one direction and coming back the opposite direction right away is a needless effort. Since this is
// the wild west, with dreadfull weather and not much water, it's important to save yourself some energy, otherwise
...
// and "SOUTH", "SOUTH" and "EAST" are not directly opposite of each other and can't become such.
// Hence the result path is itself : ["NORTH", "WEST", "SOUTH", "EAST"].
using System.Collections.Generic;

string[] a = new string[] { "NORTH", "SOUTH", "SOUTH", "EAST", "WEST", "NORTH", "WEST" }; 
string[] b = DirReduction.dirReduc(a);
Console.WriteLine(string.Join(", ", b)); // {"WEST"}

a = new string[] {"NORTH", "WEST", "SOUTH", "EAST"}; 
b = DirReduction.dirReduc(a);
Console.WriteLine(string.Join(", ", b)); // {"NORTH", "WEST", "SOUTH", "EAST"}

public class DirReduction
{
    public static string[] dirReduc(String[] arr)
    {
        List<string> way = new List<string>(arr);

        int i = 0;
        while (i < way.Count - 1)
        {
            if (
                (way[i] == "NORTH" && way[i + 1] == "SOUTH")
                || (way[i] == "SOUTH" && way[i + 1] == "NORTH")
                || (way[i] == "EAST" && way[i + 1] == "WEST")
                || (way[i] == "WEST" && way[i + 1] == "EAST")
            )
            {
                way.RemoveAt(i);
                way.RemoveAt(i);
